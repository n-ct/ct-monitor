...
func mustGetSTH(t *testing.T) (interface{}, error) {
	t.Helper()
...
func mustGetSTHWithConsistencyProof(t *testing.T, firstTreeSize, secondTreeSize uint64) (interface{}, error) {
	t.Helper()
	logClient, ctx, _ := mustCreateLogClient(t)
...
func TestGetSTHWithConsistencyProofReturnType(t *testing.T) {
	validFirstTreeSize := uint64(100)
	validSecondTreeSize := uint64(1000)
	sthpoc, _ := mustGetSTHWithConsistencyProof(t, validFirstTreeSize, validSecondTreeSize)
	if _, ok := sthpoc.(*CTObject); !ok {
		t.Fatalf("incorrect data type received from GetSTH: %T", sthpoc)
	}
}

func mustGetEntryAndProof(t *testing.T, entryIndex, treeSize uint64) (interface{}, interface{}, error) {
	t.Helper()
	logClient, ctx, _ := mustCreateLogClient(t)
...
func TestGetEntryAndProofReturnType(t *testing.T) {
	validTreeSize := uint64(100)
	validEntryIndex := uint64(4)
	poi, _, _ := mustGetEntryAndProof(t, validEntryIndex, validTreeSize)
